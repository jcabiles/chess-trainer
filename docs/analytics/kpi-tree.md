...
| **Endgame accuracy & conversion** | Accuracy over plies in the *stable* endgame suffix per material signature; conversion = endgames entered winning (≥ +2.0) that ended won | signature/suffix logic in `app/endgame.py`; aggregation + the ≥ 4-scored-moves floor (`_EG_MIN_MOVES`) in `app/insights.py` | `game_phase` is not monotone (promotion reverts middlegame) — the STABLE-suffix index guards this; a game whose endgame suffix has < 4 scored user moves is excluded from the accuracy average (it still counts toward games/conversion) |
