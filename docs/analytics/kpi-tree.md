...
|---|---|---|---|
| **Blunder rate** | Leaks with `severity='blunder'` ÷ analyzed user moves. A blunder is a move dropping the mover's win probability ≥ 20 points (`analysis.BLUNDER_WP_DROP=0.20`; mistakes ≥ 10, `MISTAKE_WP_DROP=0.10`); win prob uses the Lichess model `p = 1/(1+e^(−0.00368·cp))` (`analysis.win_prob_from_cp`) | `app/review.py` writes `leaks`; rates aggregated in `app/insights.py`, `app/profile.py` | Depth-10 evals; a deeper engine would re-label some borderline moves. Win-prob axis is tuned for ~800–1100 play by design |
| **Foreseeable-blunder %** | Fraction of leaks where the punished threat existed ≥ 2 plies before the mistake (`lead_in_ply < ply − 1`) | `app/insights.py::_foreseeable` | The strict two-plies-early rule **under-claims** deliberately: `lead_in_ply = ply−1` is the display default on every leak, so counting it would read a vacuous 100% |
| **Hope-chess rate** | Analyzed games containing ≥ 1 missed-threat leak ÷ analyzed games | `app/profile.py::_hope_chess_rate` | Game-level flag, not per-move — one bad game and one terrible game count the same |
| **Move quality labels** (play mode) | Centipawn loss between matched-limit evals before/after the move: ≤ 20 best-ish, ≤ 100 inaccuracy, ≤ 250 mistake, > 250 blunder (`analysis.classify`) | `app/analysis.py`, live `/api/move` | Different axis (cp-loss) from review severities (win-prob drop) **by design** — the two may disagree on borderline moves; play mode favors opening-prep feedback |
| **Repertoire adherence** | A game is on-repertoire when its opening moves enter a prepared line; `avg_followed_prep_depth` = plies matched before first deviation, with the deviating move + prepared answer recorded | `app/insights.py` openings slice, vs `data/repertoire.json` | Only measures lines the user *prepared* — thin repertoire ⇒ high "off-book" rate that says nothing about quality |
...
| **Endgame accuracy & conversion** | Accuracy over plies in the *stable* endgame suffix per material signature (floor: ≥ 4 user moves); conversion = endgames entered winning (≥ +2.0) that ended won | `app/endgame.py`, `insights.build_endgame_insights` | `game_phase` is not monotone (promotion reverts middlegame) — the STABLE-suffix index guards this; single-game endgames are excluded from averages |
| **Time-trouble blunder rate** | Leak rate over user moves bucketed by remaining clock (<10s, 10–30s, 30s–2m, >2m) vs the all-clocked-moves baseline | `app/insights.py::_time_trouble` over `game_plies.clock_centis` | Only games fetched/imported **with `%clk`** count; `unclocked_games` is reported instead of silently mixing. Correlation with the clock, not causation |
| **Game accuracy %** | Chess.com-style per-side accuracy: mean of per-move accuracies from win%-drop via `103.17·e^(−0.043·drop)+3.17` (`accuracy.move_accuracy`), computed on stored evals — no extra engine work | `app/accuracy.py::summarize`, shown in the review bar | Anti-vanity rule: **not** aggregated across games in Insights — single-game accuracy varies wildly with opponent resistance |
| **Estimated Elo** | Linear heuristic on game accuracy: `45·acc − 2000`, clamped [100, 2900] (`accuracy.accuracy_to_elo`) | `app/accuracy.py` | A rough single-game vibe, labeled "est." in the UI. Not a rating; never trended |
| **Trend (weekly leaks)** | Leak counts bucketed by ISO week of import | `app/profile.py::_trend` | Buckets by *import* date — a bulk import of old games spikes one week |
| **Trainer progress** | Leitner-box distribution + due counts per motif bucket in the blunder trainer | `app/trainer.py` (SQLite `trainer_*` tables) | Measures drilling activity, not transfer; the transfer question is the parked "training effectiveness" analysis (roadmap Later) |
